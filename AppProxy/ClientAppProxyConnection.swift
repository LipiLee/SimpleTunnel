...
class ClientAppProxyTCPConnection : ClientAppProxyConnection {
		...
		TCPFlow.readData { data, readError in
            guard readError != nil else {
                if let error = readError {
                    simpleTunnelLog("Failed to read data from the TCP flow. error = \(error)")
                }
	...
	override func sendData(_ data: NSData) {
        TCPFlow.write(data as Data) { error in
...
class ClientAppProxyUDPConnection : ClientAppProxyConnection {
	...
	override func handleSendResult(_ error: NSError?) {
		...
		UDPFlow.readDatagrams { datagrams, remoteEndPoints, readError in
			...
			guard let readDatagrams = datagrams, let readEndpoints = remoteEndPoints else {
				simpleTunnelLog("Datagrams or remoteEndPoints passed is nil.")
				self.handleErrorCondition(.peerReset)
				return
			}
	...
	override func sendDataWithEndPoint(_ data: NSData, host: String, port: Int) {
        ...
        UDPFlow.writeDatagrams(datagrams as [Data], sentBy: endpoints) { error in
			if let error = error {
