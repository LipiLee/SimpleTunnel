...
open class Tunnel: NSObject {
	...
	func sendDataWithEndPoint(_ data: NSData, forConnection connectionIdentifier: Int, host: String, port: Int ) {
        let properties = createMessagePropertiesForConnection(connectionIdentifier, commandType: .data, extraProperties:[
				TunnelMessageKey.Data.rawValue: data as AnyObject,
	...
	func handlePacket(_ packetData: Data) -> Bool {
		...
		switch commandType {
			case .data:
					...
					let port = properties[TunnelMessageKey.Port.rawValue] as? Int
				{
                    if let connect = connection {
                        simpleTunnelLog("Received data for connection \(connect.identifier) from \(host):\(port)")
                    } else {
                        simpleTunnelLog("Received data for connection id = nil from \(host):\(port)")
                    }
			...
			case .close:
				...
				{
                    if let connect = connection {
                        simpleTunnelLog("\(connect.identifier): closing \(closeDirection)")
                    }
                    targetConnection.closeConnection(closeDirection)
				} else {
                    if let connect = connection {
                        simpleTunnelLog("\(connect.identifier): closing reads and writes")
                    }
			...
			case .packets:
				...
				{
                    targetConnection.sendPackets(packets as [NSData], protocols: protocols)
				}
