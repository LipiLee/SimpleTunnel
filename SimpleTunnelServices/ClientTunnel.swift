...
open class ClientTunnel: Tunnel {
	...
	open func startTunnel(_ provider: NETunnelProvider) -> SimpleTunnelError? {
		...
		if let colonRange = serverAddress.rangeOfCharacter(from: CharacterSet(charactersIn: ":"), options: [], range: nil) {
			...
			let port = serverAddress.substring(with: serverAddress.index(after: colonRange.lowerBound)..<serverAddress.endIndex)

			guard !hostname.isEmpty && !port.isEmpty else {
				return .badConfiguration
			}

			endpoint = NWHostEndpoint(hostname:hostname, port:port)
		}
		else {
        ...
        provider.createUDPSession(to: endpoint, from: nil)
        connection = tcpConnection
		// Register for notificationes when the connection status changes.
		tcpConnection.addObserver(self, forKeyPath: "state", options: .initial, context: &connection)
	...
	func readNextPacket() {
		...
		targetConnection.readMinimumLength(MemoryLayout<UInt32>.size, maximumLength: MemoryLayout<UInt32>.size) { data, error in
            ...
            guard let payloadData = data else {
                simpleTunnelLog("Data passed is nil")
                self.closeTunnelWithError(SimpleTunnelError.internalError as NSError)
                return
            }

			guard payloadData.count == MemoryLayout<UInt32>.size else {
				simpleTunnelLog("Length data length (\(payloadData.count)) != sizeof(UInt32) (\(MemoryLayout<UInt32>.size)")
				self.closeTunnelWithError(SimpleTunnelError.internalError as NSError)
				return
			}

			var totalLength: UInt32 = 0
			(payloadData as NSData).getBytes(&totalLength, length: MemoryLayout<UInt32>.size)
		...
		switch commandType {
			case .openResult:
				...
				guard let targetConnection = connection,
					...
					let resultCode = TunnelConnectionOpenResult(rawValue: resultCodeNumber) else {
                        success = false
                        break
                    }
