...
class ServerConnection: Connection, StreamDelegate {
    ...
    func stream(_ stream: Stream, handle eventCode: Stream.Event) {
        if let writeStreamNotNil = writeStream {
            if (stream == writeStreamNotNil) {
				switch eventCode {
					case [.hasSpaceAvailable]:
						if !savedData.isEmpty {
							guard savedData.writeToStream(writeStreamNotNil) else {
							...
							if savedData.isEmpty {
								writeStreamNotNil.remove(from: .main, forMode: RunLoopMode.defaultRunLoopMode)
								if isClosedForWrite {
        ...
        if let readStreamNotNil = readStream {
            if stream == readStreamNotNil {
				switch eventCode {
