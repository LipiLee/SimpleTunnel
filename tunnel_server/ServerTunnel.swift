	...
	func handleBytesAvailable() -> Bool {
		...
		repeat {
			...
			if packetBytesRemaining == 0 {
				...
				toRead = UInt32.bitWidth * 8 - packetBuffer.length
			}
			else {
			...
			if packetBytesRemaining == 0 {
				...
				if packetBuffer.length == UInt32.bitWidth * 8 {
					var totalLength: UInt32 = 0
					packetBuffer.getBytes(&totalLength, length: totalLength.bitWidth * 8)

					guard totalLength <= UInt32(Tunnel.maximumMessageSize) else { return false }

					// Compute the length of the payload.
					packetBytesRemaining = Int(totalLength) - totalLength.bitWidth * 8
    ...
    override func closeTunnel() {
        ...
        if let stream = readStream {
			...
			if let data = socketData {
				...
				data.getBytes(&socket, length: socket.bitWidth * 8)
				close(socket)
			}
        }
