...
class ServerTunnelConnection: Connection {
	...
	func open() -> Bool {
        ...
        newIPv4Dictionary[SettingsKey.Address.rawValue as NSObject] = address as AnyObject
	...
	func createTUNInterface() -> Int32 {
        ...
        let connectResult = withUnsafePointer(to: &socketAddressControl) {
            connect(utunSocket, UnsafeRawPointer($0).assumingMemoryBound(to: sockaddr.self), socklen_t(MemoryLayout.size(ofValue: socketAddressControl)))
		}
	...
	func setupVirtualInterface(address: String) -> Bool {
        ...
        guard let interfaceName = getTUNInterfaceName(utunSocket: utunSocket), utunSocket >= 0 && setUTUNAddress(interfaceName, address)
			else { return false }
	...
	func readPackets() {
		...
		var iovecList = [ iovec(iov_base: &protocolNumber, iov_len: protocolNumber.bitWidth * 8), iovec(iov_base: &buffer, iov_len: buffer.count) ]
		let iovecListPointer = UnsafeBufferPointer<iovec>(start: &iovecList, count: iovecList.count)
        
        let utunSocket = getSocketHandle(from: source)
		...
		repeat {
			...
			guard readCount > protocolNumber.bitWidth + 8 else { break }
			...
			packets.append(NSData(bytes: &buffer, length: readCount - protocolNumber.bitWidth * 8))
...
//        guard let newSource = dispatch_source_create(DISPATCH_SOURCE_TYPE_READ, UInt(utunSocket), 0, dispatch_get_main_queue()) else { return }
        ...
        guard let newSource = DispatchSource.makeReadSource(fileDescriptor: utunSocket) as? DispatchSource else { return }
	...
	override func sendPackets(_ packets: [NSData], protocols: [NSNumber]) {
		guard let source = utunSource else { return }
//        let utunSocket = Int32(dispatch_source_get_handle(source))
		...
		for (index, packet) in packets.enumerated() {
			...
			var iovecList = [ iovec(iov_base: &protocolNumber, iov_len: protocolNumber.bitWidth * 8), iovec(iov_base: buffer, iov_len: packet.length) ]
			...
			else if writeCount < packet.length + protocolNumber.bitWidth * 8 {
