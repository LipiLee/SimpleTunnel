...
class PacketTunnelProvider: NEPacketTunnelProvider, TunnelDelegate, ClientTunnelConnectionDelegate {
	...
	override func startTunnel(options: [String : NSObject]?, completionHandler: @escaping (Error?) -> Void) {
        simpleTunnelLog("PacketTunnelProvider.startTunnel() start.")
	...
	func tunnelDidOpen(_ targetTunnel: Tunnel) {
        ...
        if let clientTunnel = tunnel {
            let newConnection = ClientTunnelConnection(tunnel: clientTunnel, clientPacketFlow: packetFlow, connectionDelegate: self)
            newConnection.open()
            tunnelConnection = newConnection
        } else {
            simpleTunnelLog("tunnel is not assigned.")
        }
	}
	...
	func tunnelConnectionDidOpen(_ connection: ClientTunnelConnection, configuration: [NSObject: AnyObject]) {
		...
		setTunnelNetworkSettings(settings) { errorOptional in
